...
class SourceMap:
    ...
    def get_buggy_line(self, pc):
        try:
            ...
            # Calcular o número da linha baseado na posição
...
def detect_reentrancy(sloads, calls, current_instruction):
    ...
    # Lembrar chamadas com mais de 2300 gas e onde o valor é maior que zero/simbólico ou onde o destino é simbólico
    elif current_instruction["op"] == "CALL" and sloads:
        gas = int(current_instruction["stack"][-1], 16)  # Gás da instrução
        ...
        if gas > 2300 and value > 0:
            ...
            for pc in sloads.values():
                if pc < current_instruction["pc"]:
                    # Aqui você pode obter o número da linha e o conteúdo correspondente
            ...
            for pc in calls:
                if pc < current_instruction["pc"]:
                    ...
                    if line_number != -1:
                        print(f"Reentrância detectada na linha {line_number}: {line_content}")
                    return pc
    # Limpa sloads e chamadas de transações anteriores
